...
// ── Config ────────────────────────────────────────────────────────────────────
const GOOGLE_MAPS_KEY  = 'AIzaSyDp7UhOKBinnaUo82W5tafA-QMSN1wFHB4'
...
async function getColoniasToFix() {
  return new Promise((resolve, reject) => {
    // Buscar TODAS las colonias para forzar actualización de precisión
    const path = `/rest/v1/colonias?select=id,nombre,etiqueta_zona,precio,lat,lng&order=nombre`
    const opts = {
      hostname: 'jdrrkpvodnqoljycixbg.supabase.co',
      path,
      method: 'GET',
