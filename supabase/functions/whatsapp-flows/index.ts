...
serve(async (req) => {
  if (req.method === 'POST') {
    try {
      ...
      console.log("==> FLOW REQUEST RECEIVED:", JSON.stringify(body).substring(0, 500) + '...');
      
      const FLOWS_PRIVATE_KEY = Deno.env.get('FLOW_PRIVATE_KEY') || Deno.env.get('FLOWS_PRIVATE_KEY')
      if (!FLOWS_PRIVATE_KEY) throw new Error("Llave privada de Flows no configurada en las variables de entorno")
      ...
      console.error("==> FLOW FATAL ERROR:", e.message, e.stack)
