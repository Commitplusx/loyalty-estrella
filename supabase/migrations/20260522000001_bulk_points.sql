-- Migration: 20260522000001_bulk_points.sql
-- Optimiza la inserción de múltiples puntos en una sola llamada a DB (Resuelve el N+1 Query)
...
BEGIN
  ...
  FOR i IN 1..p_cantidad LOOP
    ...
    -- LÓGICA DE ASCENSO VIP AUTOMÁTICO
    ...
    IF v_cliente.puntos_expiran_at IS NOT NULL AND NOW() > v_cliente.puntos_expiran_at AND COALESCE(v_cliente.puntos, 0) > 0 THEN
      ...
      v_cliente.puntos := 0;
      v_cliente.puntos_expiran_at := NULL;
      v_cliente.entregas_ciclo := 0;
    ...
    IF v_cliente.es_vip THEN
      v_ganancia_actual := fn_valor_entrega_vip(v_cliente.entregas_ciclo);
    ELSE
      v_ganancia_actual := 0.00;
    END IF;
    ...
    INSERT INTO movimientos_saldo (cliente_tel, tipo, descripcion, puntos_delta, saldo_delta, puntos_nuevo, saldo_nuevo, pedido_id)
    VALUES (p_cliente_tel, 'entrega', format('Entrega múltiple #%s. +1 punto', v_cliente.entregas_ciclo), 1, v_ganancia_actual, v_cliente.puntos, v_cliente.saldo_billetera, p_pedido_id);
    
    INSERT INTO registros_puntos (cliente_id, tipo, puntos, monto_saldo, descripcion)
    VALUES (v_cliente.id, 'acumulacion', 1, v_ganancia_actual, 'Entrega múltiple # ' || v_cliente.entregas_ciclo);
  END LOOP;

  UPDATE clientes
  SET puntos = v_cliente.puntos,
      saldo_billetera = v_cliente.saldo_billetera,
      puntos_expiran_at = v_cliente.puntos_expiran_at,
      entregas_ciclo = v_cliente.entregas_ciclo,
      ciclo_inicio_at = v_cliente.ciclo_inicio_at,
      envios_totales = v_cliente.envios_totales,
  ...
  RETURN jsonb_build_object(
    ...
    'saldo_billetera', v_cliente.saldo_billetera,
    'saldo_ganado', v_saldo_ganado_total,
    'es_vip', v_cliente.es_vip,
