...
export function ClienteView() {
  ...
  // Onboarding eliminado a petición del cliente
  ...
  useEffect(() => {
    if (viewState === 'result' && cliente && enviosRestantes === 0 && cliente.envios_totales > 0) {
      const t = setTimeout(() => {
        confetti({ particleCount: 120, spread: 80, origin: { y: 0.55 }, colors: ['#f97316', '#f59e0b', '#10b981', '#ea580c', '#8b5cf6'] });
        setTimeout(() => confetti({ particleCount: 60, spread: 100, origin: { y: 0.4, x: 0.2 }, colors: ['#f97316', '#fbbf24'] }), 300);
  ...
  return (
    <motion.div 
      ...
      className="min-h-screen overflow-x-hidden bg-white text-gray-900 transition-colors duration-300"
    >
      ...
      <div
        className={`fixed top-0 inset-x-0 z-50 bg-white/90 backdrop-blur-md transition-all duration-300
          ${navScrolled ? 'shadow-sm border-b border-gray-100' : ''}`}
          ...
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            <div className="flex items-center gap-3">
              <div className="w-6 h-6 flex items-center justify-center">
                <img src="/estrella-circle.png" alt="Estrella" className="w-full h-full object-contain" />
              </div>
              <div className="font-black text-lg text-gray-900 tracking-tight">
                Estrella<span className="text-orange-500">.</span>
              </div>
            </div>

            {/* Estado tienda */}
            <div className="hidden sm:flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border border-gray-200 text-gray-600">
            ...
            <div className="flex items-center gap-2">
              ...
              {cliente && (
                <Button
                  ...
                  size="sm"
                  onClick={handleReset}
                  className="text-gray-500 hover:text-gray-900 hover:bg-gray-100 ml-1 text-xs gap-1 font-bold"
                >
      ...
      <main className="max-w-6xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8 pt-20 sm:pt-24"
          ...
          <div className="space-y-6">
            ...
            <Card className="border-0 shadow-xl overflow-hidden">
              ...
              <CardContent className="p-6 space-y-4 text-center">
                ...
                <div className="flex items-center gap-3 p-4 bg-orange-50 rounded-xl text-left border border-orange-100">
        ...
        {viewState === 'search' && (
          ...
          >
            {/* ── HERO REDISEÑADO (ESTILO ESTRELLA EATS) ── */}
            <div className="relative flex flex-col items-center justify-center px-6 pt-16 pb-16 bg-white overflow-hidden">
              {/* Luces de fondo (Glow effects) */}
              <div className="absolute top-0 right-0 w-96 h-96 bg-orange-400/10 blur-[100px] rounded-full -translate-y-1/2 translate-x-1/3" />
              <div className="absolute bottom-0 left-0 w-72 h-72 bg-amber-400/10 blur-[80px] rounded-full translate-y-1/3 -translate-x-1/4" />
              
              {/* Pattern superpuesto sutil */}
              ...
              <motion.div
                ...
                className="relative z-10 w-24 h-24 flex items-center justify-center mb-6"
              >
                <img src="/estrella-circle.png" alt="Estrella" className="w-full h-full object-contain drop-shadow-sm" />
              </motion.div>
              ...
              >
                <Badge className="bg-orange-100 text-orange-600 border border-orange-200 mb-4 px-3 py-1 font-bold tracking-wide shadow-sm">
                  ESTRELLA EATS LOYALTY
                </Badge>
                <h1 className="text-4xl sm:text-5xl font-black text-gray-900 tracking-tight mb-3">
                  Tu <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-amber-500">Billetera</span> Digital
                </h1>
                <p className="text-gray-600 text-base font-medium max-w-sm mx-auto leading-relaxed">
                  Pide tu comida favorita, acumula puntos y desbloquea envíos gratis.
                </p>
              </motion.div>
              ...
              >
                {[
                  { val: '35K+', label: 'Entregas', icon: Truck },
                  { val: '5', label: 'Estrellas', icon: Star },
                  { val: '100%', label: 'Garantía', icon: Heart },
                ].map((s, i) => (
                  <div key={s.label} className="flex flex-col items-center bg-white rounded-2xl px-5 py-3 border border-gray-100 shadow-md">
                    <s.icon className="w-4 h-4 text-orange-500 mb-1" />
                    <p className="text-gray-900 font-black text-lg leading-tight">{s.val}</p>
            ...
            <motion.div
              ...
              className="flex-1 bg-gray-50 -mt-6 rounded-t-[32px] px-5 pt-8 pb-28 space-y-5"
            >
              ...
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none gap-2.5 z-10">
                  <span className="text-gray-800 font-bold text-lg">+52</span>
                ...
                <Input
                  ...
                  className="pl-[4.5rem] pr-14 h-16 text-2xl font-black rounded-2xl border-2 border-gray-200 bg-white focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all tracking-widest shadow-sm"
                  type="tel"
                  inputMode="numeric"
                  pattern="[0-9]*"
                  maxLength={10}
                ...
                <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none">
                  <span className={`text-sm font-bold tabular-nums transition-colors ${
                    telefono.length === 10 ? 'text-green-500' : 'text-gray-300'
                  }`}>
                    {telefono.length}/10
                  </span>
                </div>
              ...
              <div className="flex gap-1.5 justify-center">
                {Array.from({ length: 10 }).map((_, i) => (
                  <motion.div
                    ...
                    animate={{
                      backgroundColor: i < telefono.length ? '#f97316' : '#e5e7eb',
              ...
              <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.97 }}>
                <Button
                  ...
                  className="w-full h-12 bg-white text-gray-700 border-2 border-gray-200 font-semibold text-sm rounded-2xl hover:border-green-400 hover:text-green-700 transition-all flex items-center justify-center gap-2"
                >
              ...
              {/* ── BANNER ESTRELLA EATS (NUEVO) ── */}
              <motion.div
                initial={{ opacity: 0, y: 15 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.4 }}
                className="mt-6 p-[2px] rounded-3xl bg-gradient-to-r from-orange-400 via-amber-500 to-orange-400 shadow-lg shadow-orange-500/20"
              >
                <div className="bg-white p-5 rounded-[22px] flex flex-col sm:flex-row items-center gap-4 text-center sm:text-left">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-gray-900 text-lg">Pide en Estrella Eats</h3>
                    <p className="text-sm text-gray-500 mt-1">Nuestros restaurantes asociados te esperan. <b>Acumula puntos con cada pedido.</b></p>
                  </div>
                  <Button
                    onClick={() => window.open('https://estrella-eats.shop', '_blank')}
                    className="w-full sm:w-auto bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-xl h-11 px-6 shadow-md shadow-orange-500/30"
                  >
                    Ver Restaurantes
                  </Button>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 pt-4">
                {[
                  { icon: Gift, label: '6to gratis', sub: 'Cada 5 envíos', cls: 'text-orange-500 bg-orange-50' },
                  { icon: Star, label: 'Fidelidad', sub: 'Suma beneficios', cls: 'text-primary bg-orange-50' },
                  { icon: Truck, label: 'Garantizado', sub: 'Envío seguro', cls: 'text-green-600 bg-green-50' },
                ].map(({ icon: Icon, label, sub, cls }) => (
                  <div key={label} className="flex flex-col items-center gap-2 p-3 bg-white rounded-2xl border border-gray-100 shadow-sm text-center">
        ...
        {(viewState === 'pin-setup' || viewState === 'pin-verify') && cliente && (
          <>
            ...
            <motion.div
              ...
              className="fixed bottom-0 inset-x-0 z-40 lg:hidden bg-white rounded-t-3xl shadow-2xl"
                ...
                {viewState === 'pin-setup' ? (
                  ...
                  <>
                    <div className="text-center">
                      ...
                      <h2 className="text-2xl font-black text-gray-800 leading-tight">Ingresa tu PIN</h2>
                      <p className="text-base text-muted-foreground mt-2 font-medium">
                    ...
                    {pinAttempts > 0 && (
                      <p className="text-center text-sm font-bold text-red-500 bg-red-50 py-2.5 rounded-xl">
                ...
                {viewState === 'pin-setup' ? (
                  ...
                  <>
                    <div className="text-center">
                      ...
                      <h2 className="text-2xl font-bold text-gray-800">Ingresa tu PIN</h2>
                      <p className="text-sm text-muted-foreground mt-2">
                        Hola, <strong>{cliente.nombre || cliente.telefono}</strong> 👋
                      </p>
                    </div>
        ...
        {viewState === 'result' && cliente && (
          ...
          >
            ...
            <div className="flex items-center justify-between px-4 pb-4 lg:hidden">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleShare} className="w-9 h-9 rounded-full flex items-center justify-center bg-orange-50 text-primary">
                  <Share2 className="w-4 h-4" />
                </button>
                <button onClick={handleReset} className="w-9 h-9 rounded-full flex items-center justify-center text-gray-400 bg-gray-50">
            ...
            <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 items-start">
              ...
              <div className={`w-full lg:w-96 shrink-0 min-w-0 lg:sticky lg:top-24 space-y-6 overflow-hidden ${activeTab === 'home' ? 'block' : 'hidden lg:block'}`}>
                ...
                <div className="hidden lg:flex items-center justify-between px-2 pb-2">
                  <h2 className="text-xl font-bold text-gray-900">Mi Tarjeta VIP</h2>
                  <div className="flex items-center gap-2">
                    <button onClick={handleShare} className="w-9 h-9 rounded-full flex items-center justify-center bg-orange-50 text-primary">
                      <Share2 className="w-4 h-4" />
                    </button>
                ...
                {/* Cloudinary VIP Card */}
                <div className="relative w-full max-w-[420px] mx-auto rounded-[20px] overflow-hidden shadow-[0_20px_40px_-15px_rgba(0,0,0,0.3)] transition-transform hover:-translate-y-1 ring-1 ring-black/5">
                ...
                {/* ── BANNER SECUNDARIO ESTRELLA EATS (PERFIL) ── */}
                <div onClick={() => window.open('https://estrella-eats.shop', '_blank')} className="bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 rounded-[20px] p-4 flex items-center gap-4 cursor-pointer hover:shadow-md hover:border-orange-300 transition-all group">
                  <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shrink-0 group-hover:scale-110 group-hover:shadow-lg shadow-orange-500/20 transition-all">
                    <Utensils className="w-6 h-6 text-orange-500" />
                  </div>
                  <div className="flex-1">
                    <h4 className="font-bold text-gray-900 text-sm">¿Tienes hambre?</h4>
                    <p className="text-xs text-gray-500 mt-0.5">Pide en <b className="text-orange-600">Estrella Eats</b> y suma puntos</p>
                  </div>
                </div>
              </div>
      ...
      <footer className="hidden lg:block border-t border-gray-100 bg-white/50 backdrop-blur-sm">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-sm text-gray-700">Estrella Delivery</span>
            </div>
            <div className="flex items-center gap-1.5 text-xs text-gray-400">
              <Clock className="w-3.5 h-3.5" /> Lun - Dom: 9 AM - 10 PM
            </div>
          </div>
          <div className="mt-4 pt-4 border-t border-gray-100 text-center text-[10px] text-gray-300">
