      ...
      <section className="pt-36 sm:pt-44 pb-20 sm:pb-24 px-5 max-w-5xl mx-auto"
        ...
        <div className="grid lg:grid-cols-[1fr_340px] gap-16 items-center">
          <div>
            ...
            <motion.div variants={fadeUp} initial="hidden" animate="show" custom={3} className="w-full flex justify-center lg:justify-start">
              <form onSubmit={(e) => { 
                e.preventDefault(); 
                const tel = new FormData(e.currentTarget).get('tel'); 
                if(tel) navigate('/loyalty/' + tel); 
              }} className="flex flex-col sm:flex-row gap-3 w-full max-w-[320px] sm:max-w-[380px]">
                <div className="relative flex-1">
                  <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                  <input type="tel" name="tel" placeholder="Tu número celular..." className="w-full pl-11 pr-4 py-3 rounded-xl border-2 border-gray-200 focus:outline-none focus:ring-4 focus:ring-blue-600/20 focus:border-blue-600 text-gray-900 bg-white font-bold text-base" required minLength={10} maxLength={10} />
                </div>
                <motion.button type="submit"
                  whileHover={{ scale: 1.02, y: -1 }} whileTap={{ scale: 0.98 }}
                  className="flex items-center justify-center gap-2 bg-blue-600 text-white font-semibold px-6 py-3 rounded-xl text-sm shadow-xl shadow-blue-600/30 hover:bg-blue-700 transition-colors shrink-0">
