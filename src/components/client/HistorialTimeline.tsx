...
export function HistorialTimeline({ historial, cuponActivo }: HistorialTimelineProps) {
  ...
  return (
    <div className="mb-6">
      ...
      {/* Tabs */}
      <div className="flex gap-2 mb-5 bg-gray-100 p-1.5 rounded-2xl">
      ...
      <div className="space-y-3">
            ...
            .map((mov: RegistroMovimiento) => {
              ...
              return (
                <div key={mov.id ?? `${mov.cliente_id}-${mov.created_at}`}>
                  <Card className={`border-0 shadow-sm overflow-hidden transition-all hover:shadow-md ${isCanje && couponCode === cuponActivo ? 'ring-2 ring-amber-400/60' : 'bg-white'}`}>
                    <CardContent className="p-5 flex items-center gap-4">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-sm ${
                        isCashback ? 'bg-amber-100' 
                        : isCanje ? 'bg-emerald-100' 
                        : 'bg-orange-100'
                      }`}>
                        {isCashback
                    ...
                    {couponCode && couponCode === cuponActivo && (
                      <div className="px-5 pb-5 -mt-1">
                        <div className="relative bg-gradient-to-r from-amber-50 to-yellow-50 border-2 border-dashed border-amber-300 rounded-2xl p-4 flex items-center justify-between gap-4">
                          <div className="flex items-center gap-3 min-w-0">
                            <span className="text-2xl">🎟️</span>
                            <div className="min-w-0">
                              <p className="text-[11px] uppercase tracking-wider text-amber-600 font-bold mb-0.5">Tu código de descuento</p>
                              <p className="font-mono font-black text-amber-700 text-xl tracking-widest truncate">{couponCode}</p>
                            </div>
                          </div>
                          <button
                            ...
                            className="shrink-0 text-xs font-black text-amber-700 bg-amber-200 shadow-sm px-4 py-2.5 rounded-xl hover:bg-amber-300 transition-all active:scale-[0.95]"
                          >
                            Copiar
                          </button>
                        </div>
        ...
        {filtered.length > historialLimit && (
          <button
            ...
            className="w-full py-4 text-base font-bold text-blue-600 hover:bg-blue-50 rounded-2xl transition-all border-2 border-blue-100 shadow-sm active:scale-[0.98]"
          >
            Ver más movimientos ↓
          </button>
        )}
