...
import '../providers/optimistic_ui_provider.dart';
import '../main.dart' show stopAlarm;

// Provider para recordar pedidos rechazados localmente en esta sesión
final rejectedPedidosProvider = StateProvider<Set<String>>((ref) => {});
...
final pedidosActivosProvider = StreamProvider<List<PedidoModel>>((ref) {
  // Auto-reconectar cuando vuelva el internet
  ref.watch(connectivityProvider);
  final rejected = ref.watch(rejectedPedidosProvider);
  ...
  if (userId != null) {
    return supabase
        ...
        .limit(150)
        .asyncMap((list) async {
      // Filtrar: O está asignado a MÍ, o es del POOL (pendiente y sin asignar)
      final pedidosRows = list.where((m) {
        if (rejected.contains(m['id'])) return false;
        
        final isMine = m['repartidor_id'] == userId && m['estado'] != 'entregado' && m['estado'] != 'cancelado';
        final isPool = (m['estado'] == 'buscando_repartidor' || m['estado'] == 'preparando') && (m['repartidor_id'] == null || m['repartidor_id'] == '');
        return (isMine || isPool) && m['estado'] != 'pendiente_pago';
      }).toList();
      
      if (pedidosRows.isEmpty) {
        // Si la base de datos dice que hay 0 pedidos, limpiamos la caché guardando lista vacía
        await CacheHelper.saveList('pedidos_activos_$userId', []);
        return <PedidoModel>[];
      }

      final restIds = pedidosRows.map((p) => p['restaurante_id']).whereType<String>().toSet().toList();
      Map<String, dynamic> restaurantesMap = {};
      if (restIds.isNotEmpty) {
        try {
          final resData = await supabase.from('restaurantes').select('id, nombre, lat, lng').inFilter('id', restIds);
          for (var r in resData) restaurantesMap[r['id']] = r;
        } catch (e) {
          print('Error fetching restaurantes: $e');
        }
      }

      final result = pedidosRows.map((m) {
        final rawMap = Map<String, dynamic>.from(m);
        if (rawMap['restaurante_id'] != null && restaurantesMap.containsKey(rawMap['restaurante_id'])) {
          rawMap['restaurante'] = restaurantesMap[rawMap['restaurante_id']]['nombre'];
          rawMap['restaurantes'] = restaurantesMap[rawMap['restaurante_id']];
        }
        return PedidoModel.fromMap(rawMap);
      }).toList();

      // Guardar en caché cada vez que lleguen datos reales
      await CacheHelper.saveList('pedidos_activos_$userId', result.map((p) => p.toMap()).toList());
      return result;
    });
  } else {
    return supabase
        ...
        .limit(150)
        .asyncMap((list) async {
      final pedidosRows = list.where((m) => m['estado'] != 'pendiente_pago').toList();
      if (pedidosRows.isEmpty) return <PedidoModel>[];
      ...
      // Join con repartidores para que el admin vea el nombre del repartidor
      Map<String, dynamic> repartidoresMap = {};
      try {
        final repData = await supabase.from('repartidores').select('user_id, nombre');
        for (var r in repData) {
          if (r['user_id'] != null) {
            repartidoresMap[r['user_id']] = r;
          }
        }
      } catch (e) {
        debugPrint('Error en stream repartidores: $e');
      }

      return pedidosRows.map((m) {
        final rawMap = Map<String, dynamic>.from(m);
        if (rawMap['restaurante_id'] != null && restaurantesMap.containsKey(rawMap['restaurante_id'])) {
          rawMap['restaurante'] = restaurantesMap[rawMap['restaurante_id']]['nombre'];
          rawMap['restaurantes'] = restaurantesMap[rawMap['restaurante_id']];
        }
        if (rawMap['repartidor_id'] != null && repartidoresMap.containsKey(rawMap['repartidor_id'])) {
          rawMap['repartidores'] = repartidoresMap[rawMap['repartidor_id']];
        }
        return PedidoModel.fromMap(rawMap);
      }).toList();
    });
...
class DriverPedidosScreen extends ConsumerStatefulWidget {
  const DriverPedidosScreen({super.key});

  @override
  ConsumerState<DriverPedidosScreen> createState() => _DriverPedidosScreenState();
}

class _DriverPedidosScreenState extends ConsumerState<DriverPedidosScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final optimisticOverrides = ref.watch(optimisticMutationsProvider);
        ...
        actions: [
          if (ref.watch(isAdminProvider))
            IconButton(
              ...
              tooltip: 'Pedido de Prueba',
              onPressed: () => _crearPedidoPrueba(context, ref),
            ),
          IconButton(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text('$error', style: const TextStyle(fontSize: 13, color: Colors.black38), textAlign: TextAlign.center),
              ],
            ),
          ),
        ),
        data: (allPedidos) {
          // 1. Aplicar Overrides de Optimistic UI
          final mappedPedidos = allPedidos.map((p) {
            if (optimisticOverrides.containsKey(p.id)) {
              return p.copyWith(estado: optimisticOverrides[p.id]);
            }
            return p;
          }).toList();

          // 2. Aplicar filtro por estado
          final pedidosPorEstado = mappedPedidos.where((p) {
  ...
  Future<void> _crearPedidoPrueba(BuildContext context, WidgetRef ref) async {
    // 1. Obtener repartidores activos
    final reps = await supabase
        .from('repartidores')
        .select('id, nombre')
        .eq('activo', true)
        .order('nombre');
    ...
    if (reps.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No hay repartidores activos'), backgroundColor: Colors.orange),
      );
      return;
    }

    // 2. Pedir que elija un repartidor
    String? repId = reps.first['id']?.toString();
        ...
        builder: (ctx, setDlgState) => AlertDialog(
          ...
          title: const Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Pedido de Prueba', style: TextStyle(fontWeight: FontWeight.bold)),
            ],
          ),
          content: Column(
            ...
            children: [
              const Text(
                'Crea un pedido falso con efectivo (\$150 total, \$30 envío) para probar el flujo.\n\nNo se manda ningún WhatsApp.',
                style: TextStyle(fontSize: 13),
              ),
              const SizedBox(height: 20),
              const Text('Asignar a:', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
              const SizedBox(height: 8),
              DropdownButtonFormField<String>(
                ...
                items: reps.map<DropdownMenuItem<String>>((r) => DropdownMenuItem(
                  value: r['id']?.toString(),
                  child: Text(r['nombre'] ?? '?'),
                )).toList(),
                onChanged: (v) => setDlgState(() => repId = v),
              ),
            ],
          ),
          actions: [
            ...
            FilledButton.icon(
              onPressed: () async {
                ...
                final id = await ref.read(pedidoServiceProvider).crearPedidoPrueba(repartidorId: repId!);
                if (!context.mounted) return;
                if (id != null) {
                  ref.invalidate(pedidosActivosProvider);
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('🧪 Pedido de prueba creado — búscalo en la lista'),
                      backgroundColor: Color(0xFF10B981),
                    ),
                  );
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(content: Text('Error al crear pedido de prueba'), backgroundColor: Colors.red),
                  );
                }
              },
              icon: const Icon(Icons.science_rounded),
...
class _PedidoTile extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_PedidoTile> createState() => _PedidoTileState();
}

class _PedidoTileState extends ConsumerState<_PedidoTile> {
  ...
  void initState() {
    super.initState();
    // Refrescar cada minuto para actualizar tiempoAgo y tiempo de preparación
  ...
  @override
  Widget build(BuildContext context) {    
    if (_isHiding) return const SizedBox.shrink();
    
    final color = _estadoColor(widget.pedido.estado);
    final timeAgo = _timeAgo(widget.pedido.createdAt);
    
    int prepMinutes = widget.pedido.tiempoPreparacion ?? 0;
    int remainingPrep = 0;
    if (prepMinutes > 0) {
      final elapsed = DateTime.now().difference(widget.pedido.createdAt).inMinutes;
      remainingPrep = prepMinutes - elapsed;
      if (remainingPrep < 0) remainingPrep = 0;
    }
    
    final String origenStr = widget.pedido.restaurante?.isNotEmpty == true ? widget.pedido.restaurante! : (widget.pedido.origen?.isNotEmpty == true ? widget.pedido.origen! : 'Punto de recogida');
    final String destinoStr = widget.pedido.clienteNombre?.isNotEmpty == true ? widget.pedido.clienteNombre! : (widget.pedido.destino?.isNotEmpty == true ? widget.pedido.destino! : 'Cliente');

    return Container(
      margin: const EdgeInsets.only(bottom: 8),
      child: Dismissible(
        key: Key('pedido_${widget.pedido.id}'),
        direction: (widget.pedido.estado == 'pendiente' || widget.pedido.estado == 'asignado' || widget.pedido.estado == 'en_cocina' || widget.pedido.estado == 'listo_para_recoger' || widget.pedido.estado == 'en_camino' || widget.pedido.estado == 'recibido') 
        ...
        confirmDismiss: (direction) async {
          ...
          String nextState = '';
          if (widget.pedido.estado == 'pendiente' || widget.pedido.estado == 'pendiente_pago') nextState = 'asignado';
          else if (widget.pedido.estado == 'asignado' || widget.pedido.estado == 'en_cocina' || widget.pedido.estado == 'listo_para_recoger') nextState = 'recibido';
          else if (widget.pedido.estado == 'recibido') nextState = 'en_camino';
          else if (widget.pedido.estado == 'en_camino') nextState = 'entregado';
          
          if (nextState.isNotEmpty) {
            stopAlarm(); // Detener la alarma si suena por un pedido entrante
            
            final updateData = {'estado': nextState};
            if (widget.pedido.estado == 'pendiente' || widget.pedido.repartidorId == null) {
              updateData['repartidor_id'] = supabase.auth.currentUser?.id ?? '';
            }
            
            if (nextState == 'entregado' || nextState == 'cancelado') {
              OriginIslandService.stopIsland();
            }

            // Optimistic UI + Cola Local
            ref.read(optimisticMutationsProvider.notifier).mutateOrder(widget.pedido.id, updateData);
          }
          return false; // Devolvemos false para evitar que Flutter lance error de Dismissible, nosotros manejamos el UI
        },
        child: BouncingCard(
          onTap: () {
        ...
        child: Material(
          ...
          child: Container(
            padding: const EdgeInsets.all(16),
            decoration: BoxDecoration(
              color: (widget.pedido.estado == 'asignado' || widget.pedido.estado == 'pendiente' || widget.pedido.estado == 'pendiente_pago')
                  ? (widget.isDark ? const Color(0xFF0F172A) : const Color(0xFFEFF6FF))
              ...
              border: Border.all(
                ...
                width: (widget.pedido.estado == 'asignado' || widget.pedido.estado == 'pendiente' || widget.pedido.estado == 'pendiente_pago') ? 1.5 : 1.0,
              ),
            ),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 6),
                        Text(
                          widget.pedido.estadoLabel,
                          style: TextStyle(
                            color: widget.onSurface.withValues(alpha: 0.8), 
                            fontSize: 12, 
                            fontWeight: FontWeight.w600,
                          ),
                        ),
                        if (widget.pedido.estado == 'asignado' || widget.pedido.estado == 'pendiente' || widget.pedido.estado == 'pendiente_pago') ...[
                          const SizedBox(width: 8),
                          Container(
                        ...
                        if (widget.pedido.tipoPedido == 'tienda') ...[
                          const SizedBox(width: 8),
                          Text('•', style: TextStyle(color: widget.onSurface.withValues(alpha: 0.3), fontSize: 12)),
                          const SizedBox(width: 8),
                    ...
                    Column(
                      ...
                      children: [
                        Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            if (prepMinutes > 0 && !['recibido', 'en_camino', 'listo_para_recoger', 'entregado', 'cancelado'].contains(widget.pedido.estado)) ...[
                              Container(
                                padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                                margin: const EdgeInsets.only(right: 6),
                                decoration: BoxDecoration(
                                  color: remainingPrep > 0 ? Colors.orange.withOpacity(0.1) : Colors.green.withOpacity(0.1),
                                  borderRadius: BorderRadius.circular(6),
                                  border: Border.all(color: remainingPrep > 0 ? Colors.orange.withOpacity(0.5) : Colors.green.withOpacity(0.5)),
                                ),
                                child: Text(
                                  remainingPrep > 0 ? 'Faltan $remainingPrep min' : 'Debe estar listo',
                                  style: TextStyle(
                                    color: remainingPrep > 0 ? Colors.orange : Colors.green,
                                    fontSize: 10,
                            ...
                            Text(
                              timeAgo,
                              style: TextStyle(
                                color: timeAgo == 'ahora' ? Colors.red : widget.onSurface.withValues(alpha: 0.4),
                                fontSize: 12,
                        ...
                        Text(
                          'EST-${widget.pedido.id.replaceAll('-', '').substring(widget.pedido.id.replaceAll('-', '').length - 5).toUpperCase()}',
                          style: TextStyle(
                ...
                Row(
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            'De',
                            style: TextStyle(color: widget.onSurface.withValues(alpha: 0.4), fontSize: 11, fontWeight: FontWeight.w500),
                          ),
                          const SizedBox(height: 2),
                          Row(
                            crossAxisAlignment: CrossAxisAlignment.center,
                            children: [
                              Flexible(
                                child: Text(
                                  origenStr,
                                  style: TextStyle(
                                    color: widget.onSurface.withValues(alpha: 0.9),
                                    fontWeight: FontWeight.w600,
                                    fontSize: 15,
                                  ),
                                  maxLines: 1,
                              ...
                              const SizedBox(width: 6),
                              Hero(
                                tag: 'pedido_icon_${widget.pedido.id}',
                                child: _SourceBadge(tipoPedido: widget.pedido.tipoPedido),
                              ),
                              const SizedBox(width: 6),
                            ],
                          ),
                    ...
                    Container(width: 1, height: 30, color: widget.onSurface.withValues(alpha: 0.1), margin: const EdgeInsets.symmetric(horizontal: 16)),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                ...
                if (widget.pedido.descripcion.isNotEmpty) ...[
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: widget.isDark ? Colors.white.withValues(alpha: 0.03) : const Color(0xFFF9FAFB),
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Row(
                      children: [
                        Icon(Icons.notes_rounded, size: 14, color: widget.onSurface.withValues(alpha: 0.4)),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Text(
                ...
                // --- SMART ACTION BUTTON (MINI-ITINERARIO) ---
                if (widget.pedido.estado != 'entregado' && widget.pedido.estado != 'cancelado') ...[
                  const SizedBox(height: 16),
                  Builder(
                    builder: (context) {
                      String nextState = '';
                      ...
                      Color btnColor = const Color(0xFF10B981); // Verde por defecto
                      
                      if (widget.pedido.estado == 'pendiente' || widget.pedido.estado == 'pendiente_pago' || widget.pedido.estado == 'ofrecido') {
                        nextState = 'asignado';
                        btnLabel = 'Aceptar Pedido';
                        btnIcon = Icons.thumb_up_alt_rounded;
                        btnColor = const Color(0xFF3B82F6);
                      } else if (widget.pedido.estado == 'asignado' || widget.pedido.estado == 'en_cocina' || widget.pedido.estado == 'listo_para_recoger') {
                        nextState = 'recibido';
                        btnLabel = 'Ya lo tengo (Restaurante)';
                        btnIcon = Icons.storefront_rounded;
                        btnColor = const Color(0xFFF59E0B);
                      } else if (widget.pedido.estado == 'recibido') {
                        nextState = 'en_camino';
                        btnLabel = 'En camino al cliente';
                        btnIcon = Icons.directions_bike_rounded;
                        btnColor = const Color(0xFF8B5CF6);
                      } else if (widget.pedido.estado == 'en_camino') {
                        nextState = 'entregado';
                        btnLabel = 'Entregar al cliente';
                        btnIcon = Icons.task_alt_rounded;
                        btnColor = const Color(0xFF10B981);
                      }

                      if (nextState.isEmpty) return const SizedBox.shrink();
                      ...
                      return Material(
                        ...
                        child: InkWell(
                          onTap: () {
                            ...
                            setState(() {
                              _isHiding = true; // Ocultar optimísticamente
                            });
                            
                            supabase.from('pedidos').update(updateData).eq('id', widget.pedido.id).then((_) {
                               if (nextState == 'entregado' || nextState == 'cancelado') {
                                 OriginIslandService.stopIsland();
                               }
                            }).catchError((e) {
                          ...
                          child: Ink(
                            ...
                            child: Row(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                Icon(btnIcon, size: 18, color: btnColor),
                                const SizedBox(width: 8),
                                Text(
                                  btnLabel,
                                  style: TextStyle(
...
class NuevoPedidoSheet extends ConsumerStatefulWidget {
  final VoidCallback onCreado;
  const NuevoPedidoSheet({required this.onCreado});

  @override
  ConsumerState<NuevoPedidoSheet> createState() => _NuevoPedidoSheetState();
}

class _NuevoPedidoSheetState extends ConsumerState<NuevoPedidoSheet> {
