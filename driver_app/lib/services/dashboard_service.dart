  ...
  Future<Map<String, dynamic>> getDailyStats() async {
    ...
    for (var p in pedidos) {
      ...
      if (precio <= 0.0) {
        ...
        final match = RegExp(r'Costo Envío.*?\$\s*([0-9.]+)', caseSensitive: false, dotAll: true).firstMatch(desc);
  ...
  Future<Map<String, dynamic>> getDriverDailyStats(String repartidorId) async {
    ...
    for (var p in pedidos) {
      if (p['estado'] == 'entregado') {
        ...
        if (pagoPendiente && metodoPago.toLowerCase() == 'efectivo') {
          final costoComida = total - precioEnvio;
          if (costoComida > 0) {
            deudaEfectivo += costoComida;
          }
        }
      }
    }
