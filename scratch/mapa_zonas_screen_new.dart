...
class _MapaZonasScreenState extends ConsumerState<MapaZonasScreen> {
  ...
  Set<Polygon> _buildPolygons(List<Map<String, dynamic>> data) {
    ...
    for (var p in data) {
      final zona = p['etiqueta_zona'] as String?;
      final isSelected = _poligonoSeleccionado?['id'] == p['id'];
      
      // Aplicar filtro
      if (_zonaFiltro != null) {
        if (zona != _zonaFiltro && !(zona == 'rojo' && _zonaFiltro == 'roja')) continue;
      }
  ...
  Future<void> _guardarPoligonoMagico() async {
    ...
    final nuevaZona = await showModalBottomSheet<String>(
      context: context,
    ...
    if (nuevaZona == null || !mounted) return;
    
    setState(() => _isSaving = true);
    final precio = {'verde': 45, 'azul': 50, 'amarilla': 55, 'roja': 70, 'rojo': 70}[nuevaZona] ?? 45;
  ...
  Future<void> _onPoligonoTap(Map<String, dynamic> pol) async {
    final nueva = await showModalBottomSheet<String>(
      ...
      builder: (_) => _ZonaSelectorSheet(
        ...
        zonaActual: pol['etiqueta_zona'] as String? ?? 'rojo',
      ),
    );
    ...
    final tabla = pol['tipo'] == 'colonia' ? 'colonias' : 'zonas_kml';
    final updates = pol['tipo'] == 'colonia' 
        ? {'etiqueta_zona': nueva, 'precio': precio}
        : {'precio': precio}; // zonas_kml no tiene etiqueta_zona
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('✅ ${pol['nombre']} actualizado a zona ${nueva.toUpperCase()}'),
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: _colorDeZona(_poligonoSeleccionado!['etiqueta_zona'] as String?).withOpacity(0.5), width: 2),
                      ),
                      child: Row(
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: _colorDeZona(_poligonoSeleccionado!['etiqueta_zona'] as String?).withOpacity(0.15),
                              shape: BoxShape.circle,
                            ),
                            child: Text(_emojiDeZona(_poligonoSeleccionado!['etiqueta_zona'] as String?), style: const TextStyle(fontSize: 22)),
                          ),
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(_poligonoSeleccionado!['nombre'] as String, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                                Text('\$${_poligonoSeleccionado!['precio']} · Zona ${(_poligonoSeleccionado!['etiqueta_zona'] as String? ?? '?').toUpperCase()}', style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(0.6), fontSize: 13)),
                              ],
                            ),
                          ),
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: _colorDeZona(_poligonoSeleccionado!['etiqueta_zona'] as String?),
...
class _ZonaSelectorSheet extends StatelessWidget {
  ...
  const _ZonaSelectorSheet({required this.nombre, required this.zonaActual});
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Asignar zona a:', style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(0.5), fontSize: 12)),
            Text(nombre, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            ..._zonasDef.map((z) {
              final zId = z['id']!;
              final isActual = zonaActual == zId || (zonaActual == 'rojo' && zId == 'roja');
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Zona ${z['label']}', style: TextStyle(fontWeight: FontWeight.bold, color: color, fontSize: 15)),
                            Text('Envío: ${z['precio']}', style: TextStyle(color: theme.colorScheme.onSurface.withOpacity(0.5), fontSize: 12)),
                          ],
                        ),
                      ),
