  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text(isAdmin ? 'HACER CUENTAS' : 'MI CORTE', style: const TextStyle(fontWeight: FontWeight.bold)),
      ),
            ...
            data: (profile) {
              ...
              return Column(
                children: [
                  ...
                  // ── Tarjeta de Deuda de Efectivo (solo Admin) ────────────
                  if (isAdmin)
                    _DeudaEfectivoCard(
...
// ─────────────────────────────────────────────────────────────────────────────
// Tarjeta de Deuda de Efectivo (Admin only)
// ─────────────────────────────────────────────────────────────────────────────
class _DeudaEfectivoCard extends ConsumerStatefulWidget {
  final String repartidorId;
  final String nombre;
  const _DeudaEfectivoCard({required this.repartidorId, required this.nombre});

  @override
  ConsumerState<_DeudaEfectivoCard> createState() => _DeudaEfectivoCardState();
}

class _DeudaEfectivoCardState extends ConsumerState<_DeudaEfectivoCard> {
  double? _deuda;
  bool _loading = true;
  ...
  Future<void> _fetchDeuda() async {
    final d = await ref.read(repartidorServiceProvider).getDeudaEfectivo(widget.repartidorId);
    if (mounted) setState(() { _deuda = d; _loading = false; });
  }
  ...
  Future<void> _liquidar() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('Liquidar efectivo de ${widget.nombre}'),
        content: Text(
          'Estás confirmando que ${widget.nombre} ya te entregó el efectivo pendiente '
          '(\$${(_deuda ?? 0).toStringAsFixed(2)}).\n\n'
          'Esto marcará todos los pedidos como pagados y la deuda volverá a \$0.',
        ),
        actions: [
          TextButton(
    ...
    setState(() => _liquidando = true);
    final ok = await ref.read(repartidorServiceProvider).liquidarDeudaEfectivo(widget.repartidorId);
    if (mounted) {
      setState(() => _liquidando = false);
      if (ok) {
        await _fetchDeuda();
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('✅ Deuda liquidada correctamente'), backgroundColor: Color(0xFF10B981)),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Error al liquidar'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final deuda = _deuda ?? 0.0;
    if (deuda == 0) return const SizedBox.shrink(); // No mostrar si no debe nada
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.account_balance_wallet_rounded, color: Color(0xFFF43F5E), size: 32),
          const SizedBox(width: 14),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Efectivo pendiente de entrega',
                    style: TextStyle(fontSize: 12, color: cs.onSurfaceVariant, fontWeight: FontWeight.w600)),
                Text('\$${deuda.toStringAsFixed(2)}',
                    style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: Color(0xFFF43F5E))),
              ],
            ),
          ),
          _liquidando
              ...
              : FilledButton(
                  ...
                  child: const Text('LIQUIDAR', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900)),
                ),
        ],
      ),
    );
